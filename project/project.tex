...
Treating states in isolation ignores shared dynamics (no pooling) but 
averaging all states erases heterogeneity (full pooling).
...
\textbf{The primary question this project answers is:} Can a hierarchical Bayesian model 
that pools information across states produce more accurate unemployment forecasts than fitting each state independently? \\ 

\textbf{Secondary question:} Does including national macroeconomic covariates
...
neighbouring states is captured via an intrinsic conditionally autoregressive
...
small states with unreliable sample sizes. This shrinkage behaviour is expected from partial pooling. \\ 
...
an AR(1) likelihood that explicitly models temporal persistence. \\ 

Additionally, Torabi captures cross-state dependence through geographic proximity
...
Torabi's result that small states benefit most from pooling is what I expect as well. Posterior $\sigma_\beta$ should be small,
indicating strong pooling, and states with more volatile or idiosyncratic
...
Training: January 1976 -- December 2023 (28750 obs = 50 states $\times$ 48 years - 1 month (Oct 2025)) \\
Test: January 2024 -- December 2025 (1200 obs = 50 states $\times$ 24 months) \\ 

The training period spans three disruptions: national unemployment rate peaked at
10.8\% (Volcker 1982), 10.0\% (Great Financial Crisis 2008/2009), and 14.7\% (COVID 2020).
...
On Model 1, ADVI converges and the posterior means agree closely with HMC, but
credible intervals are found to be tighter but this is expected because of the
independence assumption $\prod_i q_i$ underestimating posterior
variance. \\ 

On Model 2, mean-field ADVI fails entirely: the algorithm reaches the
maximum number of dropped gradient evaluations before completing stochastic
gradient ascent, and fullrank ADVI diverges after 10{,}000 iterations.
...
of the mean-field family $\mathscr{Q}$ can represent, making the ELBO
...
% -----------------------------------------------------------------------
\section{Results}
The following results are based on HMC inference, as VI fails on Model 2.

\subsection{Convergence Diagnostics}

\textbf{Model~1:} $\hat{R}\in[0.9995,1.006]$, bulk
ESS$\in[4489,10991]$, zero divergences. \\ 
\textbf{Model~2:} $\hat{R}\in[0.9994,1.009]$, bulk
ESS$\in[578,7378]$, zero divergences. \\ 
All $\hat{R}<1.01$, indicating the 4 chains have converged to the
...
\subsection{Goodness of Fit and Shrinkage}

We assess model fit using \textbf{posterior predictive checks}: 
...
The \textbf{shrinkage plot} compares $\hat\beta_s$ (posterior mean)
from Model~2 versus Model~1 for all 50~states.
All states fall below the diagonal so the hierarchical prior shrinks
every state's AR persistence toward $\mu_\beta=0.966$.
States with the most extreme Model 1 estimates (HI, NH, NV) are
shrunk most, consistent with the theory that noisier estimates
receive stronger regularisation from the hyperprior.
...
states with different persistence patterns.
This is not evidence of misspecification as both models pass PPC
...
the prior and the strong pooling result is robust.

\textbf{Secondary question:} Both covariates are precisely estimated with
economically meaningful signs ($\gamma_{\mathrm{gdp}} = -0.034$, $\gamma_{\mathrm{fed}} = 0.021$), confirming that national macroeconomic conditions contribute meaningful
